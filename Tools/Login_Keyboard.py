...
class _PopupKeyboard(Toplevel):
    ...
    def __init__(self, parent, attach, x, y, keycolor, keysize=5):
        ...
        y_cordinate = int((screen_height/2) + (height/0.8))
    ...
    def _init_keys(self):
        self.alpha = {
            ...
            'row4' : ['@','#','%','*','[ space ]','+','-','=','@gmail.com']
            }
        
        for row in self.alpha.keys(): # iterate over dictionary of rows
            if row == 'row1':             # TO-DO: re-write this method
                i = 1                     # for readability and functionality
                for k in self.alpha[row]:
                    Button(self.row1,
                           text=k, font=("TkMenutext_font", 15),
                           width=self.keysize,
                           bg=self.keycolor,
...
class KeyboardEntry(Frame):
    ...
    def __init__(self, parent, keysize=5, keycolor='gray', *args, textvariable: tkinter.Variable = None, **kwargs):
        ...
        self.entry = Entry(self, *args, textvariable=self.textvariable, **kwargs)
        self.entry.pack()
